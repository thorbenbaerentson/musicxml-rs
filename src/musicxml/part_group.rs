...
pub struct GroupDisplay {
    ...
    #[serde(rename = "accidental-text", default = "Accidental::default")]
    pub accidental_text : Accidental,
...
mod tests {
    use super::PartGroup;
    use crate::musicxml::{
        accidental::Accidental, barline::Barline, group_barline::GroupBarline, group_symbol::GroupSymbol, note::StartStop, part_group::GroupDisplay, printable_value::PrintableValue
    };
    ...
    fn group_name() {
        ...
        assert_eq!(item.accidental_text, Accidental::Flat);
    }
    ...
    fn part_group() {
        ...
        assert_eq!(name.accidental_text, Accidental::Flat);

        assert_eq!(&item.group_abbreviation.unwrap(), &"Trp. in B flat".to_string());

        let abbre = item.group_abbreviation_display.unwrap();
        assert_eq!(abbre.display_text, "Trp. in B".to_string());
        assert_eq!(abbre.accidental_text, Accidental::Flat);
